...
def cobyqa_bounds_from_hp(cs_hp):
...
def convert_problem_to_cobyqa(problem):
    xl, xu = [], []
    for cs_hp in problem.space.get_hyperparameters():
        lower, upper = cobyqa_bounds_from_hp(cs_hp)
        xl.append(lower)
        xu.append(upper)
    return xl, xu
...
class COBYQA(Search):
    """Wrapper for COBYQA derivative-free optimizater.

    $ git clone git@github.com:cobyqa/cobyqa.git
    $ cd cobyqa/
    $ pip install -e.
