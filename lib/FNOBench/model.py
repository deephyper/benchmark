...
def build_and_train_model(config: dict, device="cpu", verbose: bool = 0):
    ...
    (
        ...
        n_train=default_config["data_n_train"],
        batch_size=default_config["data_batch_size"],
        positional_encoding=default_config["data_positional_encoding"],
        test_resolutions=default_config["data_test_resolutions"],
        n_tests=default_config["data_n_tests"],
        test_batch_sizes=default_config["data_test_batch_sizes"],
        encode_input=default_config["data_encode_input"],
        encode_output=default_config["data_encode_output"],
    )

    model = TFNO2d(
        data_channels=default_config["data_channels"],
        n_modes_height=default_config["n_modes_height"],
        n_modes_width=default_config["n_modes_width"],
        n_layers=default_config["n_layers"],
        lifting_channels=default_config["lifting_channels"],
        hidden_channels=default_config["hidden_channels"],
        projection_channels=default_config["projection_channels"],
        factorization=default_config["factorization"],
        rank=default_config["rank"],
        use_mlp=default_config["use_mlp"],
        mlp=default_config["mlp"],
        skip=default_config["skip"],
        implementation=default_config["implementation"],
        stabilizer=default_config["stabilizer"],
        fno_block_precision=default_config["fno_block_precision"],
    )
    ...
    if default_config["opt_training_loss"] == "l2":
        ...
        raise ValueError(
            f"Got training_loss={default_config['opt_training_loss']} but expected one of ['l2', 'h1']"
        )
