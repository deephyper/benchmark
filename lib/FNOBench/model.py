...
def build_and_train_model(config: dict, device="cpu", verbose: bool = 0):
    ...
    (
        ...
        n_train=default_config["n_train"],
    ...
    model = TFNO(
        n_modes=(default_config["n_modes_height"], default_config["n_modes_width"]),
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        optimizer, T_max=default_config["scheduler_T_max"]
    )
    ...
    trainer = Trainer(
        ...
        device=device,
        # mg_patching_levels=default_config["mg_patching_levels"],
        wandb_log=False,
        use_distributed=False,
        verbose=True,
    )
...
def bestevaluate(best_config, device="cpu"):
