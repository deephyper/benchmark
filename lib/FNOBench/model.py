...
def build_and_train_model(config: dict, device="cpu", verbose: bool = 0):
    ...
    trainer = Trainer(model, n_epochs=default_config['opt_n_epochs'], device=device,
                      ...
                      log_output=True, use_distributed=False, verbose=default_config['verbose'])
...
def bestevaluate(best_config):
    ...
    (train_loader, validation_loader, test_loaders, output_encoder) = load_darcy_flow_small(
        n_train=best_config['data_n_train'], batch_size=best_config['data_batch_size'],
        positional_encoding=best_config['data_positional_encoding'],
        test_resolutions=[16, 32], n_tests=[100, 50],
        test_batch_sizes=[32, 32],
        encode_input=best_config['data_encode_input'], encode_output=best_config['data_encode_output'],
        )
    ...
    trainer = Trainer(best_model, n_epochs=best_config['opt_n_epochs'],
                      device=device,
    ...
    for loader_name, loader in test_loaders.items():
        ...
        h1loss = H1Loss(d=2)
        eval_losses = {'h1': h1loss, 'l2': l2loss}
        errors = trainer.evaluate(best_model, eval_losses, loader, output_encoder, log_prefix=loader_name)
        for loss_name, loss_value in errors.items():
