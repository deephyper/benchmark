...
class dtlz1(sim_func):
    ...
    def __call__(self, x):
        ...
        ker = g1(self.n, self.o, self.offset)
...
class dtlz2(sim_func):
    ...
    def __call__(self, x):
        ...
        xx = unpack(x, self.des_type)
        # Initialize kernel function
        ker = g2(self.n, self.o, self.offset)
        # Initialize output array
        fx = np.zeros(self.o)
        fx[:] = (1.0 + ker(xx)[0])
        # Calculate the output array
...
class dtlz6(sim_func):
    ...
    def __call__(self, x):
        ...
        xx = unpack(x, self.des_type)
        # Initialize kernel function
        ker = g3(self.n, self.o, self.offset)
        # Calculate theta values
        theta = np.zeros(self.o - 1)
        g3x = ker(xx)
        for i in range(self.o - 1):
...
class dtlz7(sim_func):
    ...
    def __call__(self, x):
        ...
        xx = unpack(x, self.des_type)
        # Initialize kernel function
        ker = g4(self.n, self.o, self.offset)
