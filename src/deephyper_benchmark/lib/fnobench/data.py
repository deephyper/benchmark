...
# this function returns the train_loader, validation_loader, test_loader and output encoder for the darcy_flow dataset
# it was originally written in the neuralop codes but we slightly edit it to include the validation_loader
# which is 0.2% of the whole training_data
...
def load_darcy_pt(
    ...
    x_train = (
        data["x"][0 : n_train - n_val, :, :]
        .unsqueeze(channel_dim)
        .type(torch.float32)
        .clone()
    )
    y_train = data["y"][0 : n_train - n_val, :, :].unsqueeze(channel_dim).clone()
    x_val = (
        data["x"][n_train - n_val : n_train, :, :]
        .unsqueeze(channel_dim)
        .type(torch.float32)
        .clone()
    )
    y_val = data["y"][n_train - n_val : n_train, :, :].unsqueeze(channel_dim).clone()
    del data
    ...
    if encode_output:
        ...
        # reduce_d = list(range(y_val.ndim))
        elif encoding == "pixel-wise":
            reduce_dims = [0]
        # reduce_d=[0]
