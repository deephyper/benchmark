"""Objective function implementations of the DTLZ test suite.

Derived from the implementations in ParMOO:
...
class __g1__(__dtlz_base__):
    ...
    def __call__(self, x):
        ...
        return (
            ...
            - self.o
            + np.sum(
                (x[self.o - 1 : self.n] - self.offset) ** 2
                - np.cos(20.0 * np.pi * (x[self.o - 1 : self.n] - self.offset))
            )
        ) * 100.0
...
class __g4__(__dtlz_base__):
    ...
    def __call__(self, x):
        ...
        """
        return (
            9 * np.sum(np.abs(x[self.o - 1 : self.n] - self.offset)) / float(self.n + 1 - self.o)
        ) + 1.0
...
class dtlz7(__dtlz_base__):
    ...
    def __call__(self, x):
        ...
        gx = 1.0 + ker(x)
        hx = -np.sum(x[: self.o - 1] * (1.0 + np.sin(3.0 * np.pi * x[: self.o - 1])) / gx) + float(
            self.o
        )
